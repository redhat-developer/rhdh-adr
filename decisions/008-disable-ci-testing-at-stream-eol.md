...
**Problem**: Unclear whether CI testing for an RHDH minor stream should keep running (or be restored) after published End of Life when customers have support exceptions.
...
**Scope of “CI testing” in this ADR:** **E2E nightly** jobs for that minor stream (including related test-channel workloads) **and PR checks that live as CI jobs** for that stream. Konflux / productization pipelines are out of scope (see below).
