...
The `go` directive on the main branch targets one major version behind the latest upstream Go release. Go's [release policy](https://go.dev/doc/devel/release) supports the two most recent major versions, so this keeps the module within the supported window while avoiding early-adoption risk from a newly released version.

Updates to the `go` directive are triggered by go-toolset availability, not by upstream Go release dates. The Red Hat go-toolset image typically lags behind official upstream Go releases by several months. A new Go version is considered available for the operator when a go-toolset image containing that version appears in the [Red Hat Ecosystem Catalog](https://catalog.redhat.com/en/software/containers/ubi9/go-toolset/61e5c00b4ec9945c18787690). Maintainers can monitor availability by checking the catalog for new tags or by observing when Renovate opens a Docker minor update pull request for the go-toolset image in the operator repository.
...
- [RHIDP-12347](https://redhat.atlassian.net/browse/RHIDP-12347) — prior discussion that established the policy precedent
- [RHIDP-12020](https://redhat.atlassian.net/browse/RHIDP-12020) — tracking issue for updating the operator to Go 1.26
