...
- **Egress policies**: Allow outbound traffic for:
  ...
  - Known plugin registries used by the default RHDH configuration (e.g., registry.access.redhat.com, quay.io for OCI plugins, npm.registry.redhat.com for NPM plugins, and potentially ghcr.io or the public NPM registry). Users with custom or internal mirror registries (e.g., in airgapped environments) will need to add their own policies for those endpoints
  - Any additional egress (SCM providers, CI/CD systems, auth providers, corporate proxy endpoints configured via HTTP_PROXY/HTTPS_PROXY, etc.) is left to the user to allow via their own additive NetworkPolicy resources, since these destinations are deployment-specific
- **Clear boundary between base and user-managed policies**: The base policies shipped by the Operator and Helm chart cover only the core traffic flows that are common to all RHDH deployments (DNS, API server, PostgreSQL, known OCI plugin registries, Lightspeed model endpoints, Orchestrator cross-namespace traffic). All deployment-specific egress (SCM providers, CI/CD systems, auth providers, internal mirrors, external SonataFlowPlatform endpoints, etc.) is the user's responsibility to allow.
