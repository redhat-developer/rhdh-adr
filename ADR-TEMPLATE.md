# ADR: [Short Title of Decision]

## Context

**Problem**: [What problem are we solving? What is the current limitation or pain point?]

[Describe the current situation and why a decision is needed. Include:
- What is not working or missing?
- Who is impacted?
- What constraints or requirements exist?]

## Decision

[What are we going to do? State the decision clearly and concisely.]

**Implementation approach**:
- [Key aspect 1]
- [Key aspect 2]
- [Key aspect 3]

[Optional: Include brief code examples or configuration snippets if helpful]

## Alternatives Considered

### Alternative 1: [Name]
- **Approach**: [Brief description]
- **Rejected because**: [Why this wasn't chosen]

### Alternative 2: [Name]
- **Approach**: [Brief description]
- **Rejected because**: [Why this wasn't chosen]

## Consequences

### Positive
- ✅ [Benefit 1]
- ✅ [Benefit 2]
- ✅ [Benefit 3]

### Negative
- ❌ [Trade-off or limitation 1]
- ❌ [Trade-off or limitation 2]

### Neutral
- ⚖️ [Neutral consequence 1]
- ⚖️ [Neutral consequence 2]
